...
class ConsoleReader(StoppableThread):
    ...
    def __init__(self, console, event_queue):
        ...
        # sys.stdout = open('ESP32.log')
        # sys.stderr = open('ESP32err.log')
...
class Monitor(object):
    ...
    def __init__(self, serial_instance, elf_file, make="make", toolchain_prefix=DEFAULT_TOOLCHAIN_PREFIX, eol="CRLF"):
        ...
        # subp = subprocess.Popen(miniterm.Console())
        # self.console = subp
        # self.console = miniterm.Console()
        # self.console.output = open("ESP32.log", 'w')
        filename = "logs/"+str(datetime.now()) + ".log"
        self.f = open(filename, 'w')
        # sys.stdout = f
        # f = self.console.output
        # self.console.output = f
        # sys.stderr = open("ESP32err.log", 'w')
        if os.name == 'nt':
    ...
    def main_loop(self):
        ...
        try:
            while self.serial_reader.alive: #self.console_reader.alive and
    ...
    def handle_serial_input(self, data):
        ...
        # ansi_esc = re.compile(r'\e(\[0;\d{2}m)*')
        # normal_print(ansi_esc.sub('', data))
        # self.f.write(ansi_esc.sub('', data))
        ...
        for b in data:
            if b == b'\n': # end of line
                ...
                normal_print(ansi_escape.sub('', self._read_line))
                self.f.write(ansi_escape.sub('', self._read_line))
                self._read_line = b""
            else:
                self._read_line += b
