...
class Monitor(object):
    ...
    def __init__(self, serial_instance, elf_file, make="make", toolchain_prefix=DEFAULT_TOOLCHAIN_PREFIX, eol="CRLF"):
        ...
        filename = "logs/" + str(datetime.now()).replace(" ", "").replace(":", "") + ".log"
        # filename = filename.strip()
        self.f = open(filename, 'w',buffering=31)
        if os.name == 'nt':
    ...
    def handle_serial_input(self, data):
        ...
        for b in data:
            ...
            if b == b'\n': # end of line
                ...
                self.f.write( ansi_escape.sub('', self._read_line.strip()))
                self._read_line = b""
            else:
                self._read_line += b
...
--- Press any other key to resume monitor (resets target).""".format(reason,
    ...
    def lookup_pc_address(self, pc_addr):
        ...
        self.f.write(cr_escape.sub('', translation))
